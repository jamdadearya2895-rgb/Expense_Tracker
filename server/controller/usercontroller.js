...
export const updateUser = async (req, res) => {
  try {
    const { userId } = req.user; // Ensure integer
    ...
    const result = await pool.query({
      text: `UPDATE tbluser 
             SET firstname=$1, lastname=$2, country=$3, currency=$4, contact=$5, updatedat=CURRENT_TIMESTAMP 
             WHERE id=$6 
             RETURNING *`,
    ...
    console.log(error); // This will log the exact SQL or runtime error
    res.status(500).json({ status: "failed", message: error.message });
  }
};
